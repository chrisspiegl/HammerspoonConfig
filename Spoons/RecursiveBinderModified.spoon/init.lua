-- MODIFICATIONS
-- Recursive Binder take an optional function which should be executed onClose / onTerminate
...
function obj.recursiveBind(keymap, onClose)
   if type(keymap) == 'function' then
      -- in this case "keymap" is actuall a function
      return function()
         keymap()
         if onClose and type(onClose) == 'function' then onClose() end
      end
   end
